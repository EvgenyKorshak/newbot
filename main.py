...
def menu(message):
    ...
    btn4 = types.InlineKeyboardButton("Покупки", callback_data="fines")
    btn5 = types.InlineKeyboardButton("Списания", callback_data="expenses")
...
def callback_message(call):
    if call.data == "register":
        ...
        conn = sqlite3.connect('bot.sql')
        cur = conn.cursor()
        cur.execute("SELECT enrollment.id, u1.firstname,u1.lastname, name, cost, addDate, u2.lastname, u2.firstname, comment FROM enrollment JOIN users u1 ON u1.id = enrollment.idUser JOIN users u2 ON u2.id = enrollment.idAdd order by addDate desc")
        price = "<i><b>Начислено</b></i>\n\n"
        data=cur.fetchall()
        for t in data:
            price += f'<b>{t[0]}</b>. {t[1]} {t[2]} \n<b>Вид:</b> {t[3]} \n<b>Сумма:</b> {t[4]} \n<b>Дата начисления:</b> {t[5]} \n<b>Кто начислил:</b> {t[6]} {t[7]} \n<b>Комментарий:</b> {t[8]}\n'
            for i in range(0, 30):
                price +="="
            price +='\n'
